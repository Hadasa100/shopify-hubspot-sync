...
export async function deleteHubSpotProduct(product, log) {
  ...
  try {
    ...
    if (searchResponse.results?.length > 0) {
      ...
      log(`🗑️ Deleted product from HubSpot (Shopify ID: ${shopifyId})`, true);
      return { deleted: true, sku: result.properties.hs_sku || '', title: 'Deleted Product', status: 'deleted' };
    } else {
      const msg = `⚠️ No matching HubSpot product found for deletion (Shopify ID: ${shopifyId}).`;
      log(msg, true);
      return { deleted: false, sku: '', reason: 'No matching product found' };
    }
  } catch (error) {
    const msg = `❌ Error deleting product (Shopify ID: ${shopifyId}): ${error.message || JSON.stringify(error)}`;
    log(msg, true);
