...
export async function createOrUpdateHubSpotProduct(product, log) {
  try {
    ...
    const properties = {
      ...
      ...metafields, // merges all metafield keys
    };
    ...
    // 2) Search for an existing HubSpot product by SKU (or shopify_id if you prefer)
    ...
    log(
      `❌ Failed: ${product.title} (SKU: ${product.variants?.edges?.[0]?.node?.sku || ""
