...
function parseHubSpotError(error) {
  ...
  if (errorDetail && typeof errorDetail === 'object' && Array.isArray(errorDetail.errors)) {
    errorDetail.errors.forEach((err) => {
      ...
      if (err.code === 'PROPERTY_DOESNT_EXIST' && property) {
        missingProperties.push(property);
      }

      if (err.code === 'INVALID_OPTION' && property) {
        const invalidValue = errorDetail.propertyValue || 'Invalid value';

        // Extract allowed options (they're embedded in the message as value: "XYZ")
        const matches = err.message.match(/value:\s+"(.*?)"/g) || [];
        const allowedValues = matches.map((m) => m.replace(/value:\s+"/, '').replace(/"/g, ''));

        message = `Property '${property}' does not support value '${invalidValue}'.`;
  ...
  if (missingProperties.length > 0) {
    ...
    message = `Invalid number value${property ? ` for property '${property}'` : ''}.`;
  }
