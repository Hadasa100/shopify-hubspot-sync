 ...
 */
export async function createOrUpdateHubSpotProduct(product, log, productSku) {
  try {
    ...
    if (missingProperties.length > 0) {
      log(
      `❌ Failed:  (SKU: ${productSku || ""}) - Missing Properties: ${missingProperties.join(
          ", "
        )}. ${errorMessage}`
      );
      log(
      ...
      log(
        `❌ Failed:  (SKU: ${productSku || ""}) - SKU already in use. ${errorMessage}`
      );
      log(
        `❌ Failed:  (SKU: ${productSku || ""}) - SKU already in use.`, true
      );
    } else {
      log(
        `❌ Failed:  (SKU: ${productSku || ""}) - ${errorMessage}`, true
      );
    }
  }
}
