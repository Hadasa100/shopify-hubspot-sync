...
export async function deleteHubSpotProduct(product, log) {
  ...
  if (!sku) {
    log(`❌ No SKU found for product ID: ${product.id}. Cannot delete.`);
    return;
  }
  try {
    const searchResponse = await hubspotClient.crm.products.searchApi.doSearch({
    ...
    if (searchResponse.results.length > 0) {
      ...
      log(`No matching product found for deletion (SKU: ${sku})`);
    }
  } catch (error) {
    log(`❌ Error deleting product ${product.title} (SKU: ${sku}) - ${JSON.stringify(error.body || error)}`);
  }
}
