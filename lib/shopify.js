...
/**
 * Fetch the first variant SKU of a product by its GraphQL ID.
 */
export async function getProductSKU(ownerId) {
  const query = `
    query product($id: ID!) {
      product(id: $id) {
  ...
  `;
  const variables = { id: ownerId };
  const response = await shopifyGraphQL(query, variables);
  const skuEdge = response.data.product.variants.edges[0];
  return skuEdge?.node?.sku || '';
}
...
export async function upsertProductMetafield(ownerId, namespace, key, value) {
  const lowerVal = String(value).toLowerCase();
  const type = (lowerVal === 'true' || lowerVal === 'false') ? 'boolean' : 'string';
  const mutation = `
  ...
  `;
  const variables = { metafields: [{ ownerId, namespace, key, type, value: String(value) }] };
  const result = await shopifyGraphQL(mutation, variables);
  const errors = result.errors || result.data.metafieldsSet.userErrors;
  if (errors && errors.length) {
    console.error('Shopify upsert error:', errors);
    throw new Error('Failed to upsert metafield');
  }
